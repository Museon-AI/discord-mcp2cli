...
`full` advertises the existing 210-tool catalog for maximum client
...
The current catalogues classify all 210 tools. Fifteen contracts are
colocated directly on tool definitions; a reviewed central registry covers all
209 Discord routes for credential-free reports. The `users_create_dm` route is
