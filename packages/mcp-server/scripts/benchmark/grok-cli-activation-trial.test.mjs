...
describe('Grok CLI activation trial guard', () => {
  ...
  it('binds operator consent to host, release, and trial', () => {
    const value = validateGrokCliActivationRequest({
      release: '0.26.1-museon.1',
      runId: 'run-001',
      trialId: 'trial-001',
      hostVersion: '1.2.3',
      sourceCommit: 'a'.repeat(40),
      target,
      operatorConfirmation: 'APPROVE_GROK_CLI_ACTIVATION:0.26.1-museon.1:trial-001',
      token: 'discord-token',
      executionMode: 'test',
    });
  ...
  it('builds a pinned guided setup command and rejects config drift', () => {
      ...
      mcp_servers: {
        'discord-mcp': {
          ...
          args: [
            '--yes',
            '--loglevel=error',
            '@discord-mcp/cli@0.26.1-museon.1',
            'serve',
            '--profile',
            'devbot',
          ],
    ...
    expect(() =>
      assertGrokCliConfigReady(config, { release: '0.26.1-museon.1', profile: 'devbot' }),
    ).toThrow('pinned');
    expect(() =>
      assertGrokCliConfigReady(
        '[permission]\nrules = []\n\n[mcp_servers.discord-mcp]\ncommand = "npx"\n',
        { release: '0.26.1-museon.1', profile: 'devbot' },
      ),
